...
### 2. 安裝必要的 Python 套件
建議您使用虛擬環境 (Virtual Environment)。然後安裝 `requirements.txt` 內的所有依賴套件：
```bash
pip install -r requirements.txt
```

### 3. 設定環境變數 (API Key)
本機器人使用 Google Gemini 的服務，您需要一組 API 金鑰。
請在專案根目錄下建立一個名為 `.env` 的檔案，並填入以下內容：
```env
GEMINI_API_KEY=您的_GOOGLE_GEMINI_API_KEY_填在這裡
```
> 您可以前往 [Google AI Studio](https://aistudio.google.com/app/apikey) 免費申請一組金鑰。

### 4. 啟動機器人 🚀

#### 方式 A：啟動網頁 GUI 介面 (強烈推薦)
執行以下指令，Gradio 會在本機啟動網頁伺服器：
```bash
python app.py
```
> 終端機會顯示一個 Local URL (通常是 `http://127.0.0.1:7860/`)，請點擊或複製到瀏覽器開啟。
...
```
在終端機模式下支援以下特殊指令：
- **傳送圖片**：`/image <圖片本機路徑> <你的提問>`
- **讀取檔案**：`/file <文件本機路徑> <你的提問>`
- **離開**：輸入 `exit` 或 `quit`
...
一、從模組化思維看系統整合
這次參與 AI Chatbot 專案，讓我對系統架構與資料流的整合有了更實務的體會。過去學習程式語言與各種演算法時，多半是專注於解決單一問題；但在這個專案中，我們需要將文字處理、圖片（Base64）編碼、PDF 文本提取等多種功能，完美整合進同一個 LangChain 處理流程中。這非常考驗程式碼的模組化設計，也讓我深刻體會到保持各個模組低耦合、高內聚的重要性。

二、狀態管理與資料持久化 (JSON Logging)
專案中實作「多輪對話記憶」與「自動儲存對話紀錄」的部分讓我印象特別深刻。為了讓 AI 能理解上下文，我們必須在記憶體中妥善管理歷史訊息的資料結構。同時，將這些對話狀態連同時間戳記與檔案資訊，精準且結構化地匯出成 JSON 格式，不僅大幅提升了後續除錯的效率，也為未來若要進一步串接關聯式或非關聯式資料庫（Database）儲存用戶行為，打下了非常穩固的基礎。

三、API 串接與例外處理的實戰經驗
在實際呼叫 Google Gemini API 及建置環境的過程中，遇到了不少狀況，例如 Git 路徑未設定導致的環境問題，或是 API 溝通時可能的延遲與格式限制。這些踩坑經驗讓我學到，開發一套完整的系統不能只考慮「理想狀態」，更要為各種網路異常、檔案讀取錯誤預先寫好例外處理（Exception Handling），確保系統的強健性與使用者的順暢體驗。

四、結語
看著專案從單純的命令列（CLI）腳本，一步步擴充功能，最終進化成具備圖形化介面（Web GUI）的多模態 AI 助理，不僅充滿成就感，也大幅提升了我的 Python 實戰開發能力。這次專案讓我更懂得如何將平時累積的資訊工程理論，轉化為實際解決問題的軟體應用，是一趟非常充實的學習旅程
